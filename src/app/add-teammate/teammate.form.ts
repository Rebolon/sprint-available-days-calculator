...
export class TeammateForm extends FormGroup {
  ...
  readonly availableDaysInAWeek = this.get(
    'availableDaysInAWeek'
  ) as FormControl<number>;
  readonly holidaysForNextSprint = this.get(
    'holidaysForNextSprint'
  ) as FormControl<number>;
  readonly meetingDaysAWeek = this.get(
    'meetingDaysAWeek'
  ) as FormControl<number>;
  ...
  constructor(
    ...
    super(
      formBuilder.group(
        {
          name: new FormControl<string>(teammate?.name, [
            Validators.required,
          ...
          isNewComer: new FormControl<boolean>(teammate?.isNewComer),
        } /*,
      {
        updateOn: "blur"
      }*/
